...
class DensePoseList(object):
    ...
    def __init__(
        self, densepose_datas, boxes_xyxy_abs, image_size_hw, device=_TORCH_DEVICE_CPU
    ):
        ...
        for densepose_data in densepose_datas:
            assert (
                ...
                "Attempt to initialize DensePoseList with DensePose datas "
                "of type {}, expected DensePoseDataRelative".format(
                    type(densepose_data)
                )
            )
    ...
    def to(self, device):
        ...
        return DensePoseList(
            self.densepose_datas, self.boxes_xyxy_abs, self.image_size_hw, device
        )
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            ...
            densepose_datas_rel = [
                self.densepose_datas[i] for i, x in enumerate(item) if x > 0
            ]
            boxes_xyxy_abs = self.boxes_xyxy_abs[item]
            return DensePoseList(
                densepose_datas_rel, boxes_xyxy_abs, self.image_size_hw, self.device
